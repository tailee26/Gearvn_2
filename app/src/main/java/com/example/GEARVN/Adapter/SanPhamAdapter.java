...
import com.example.GEARVN.Model.SanPhamModels;
import com.example.GEARVN.Presenter.SetOnItemClick;
import com.example.GEARVN.R;
import com.example.GEARVN.View.Bill.ContentProDuctActivity;
...
public class SanPhamAdapter  extends RecyclerView.Adapter<SanPhamAdapter.ViewHodler> {
    ...
    public SanPhamAdapter.ViewHodler onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        if(type==0){
            ...
            view = LayoutInflater.from(context).inflate(R.layout.dongsanphamchinhhang,parent,false);
        }else if(type ==4){
            view = LayoutInflater.from(context).inflate(R.layout.dong_sanphamgoiy2,parent,false);
        }else if(type ==5){
            view = LayoutInflater.from(context).inflate(R.layout.dong_sanphamhot,parent,false);
        }else if(type ==6){
            view = LayoutInflater.from(context).inflate(R.layout.dong_sanphamyeuthich,parent,false);
        }else if(type ==7){
            view = LayoutInflater.from(context).inflate(R.layout.dong_sanphamhotdeal,parent,false);
        }else if(type ==8){
